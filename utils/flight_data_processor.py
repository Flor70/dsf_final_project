...
def merge_weekend_flight_data(directory_path: str, num_cheapest_flights: int = 3) -> str:
    ...
    cheapest_flights = combined_df.sort_values(
        'price').head(num_cheapest_flights)
    ...
    for _, flight in cheapest_flights.iterrows():
        ...
        flight_dict['search_timestamp'] = datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S')

        # Clean up the dictionary (remove NaN values)
        flight_dict = {k: ('' if pd.isna(v) else v)
                       for k, v in flight_dict.items()}

        result.append(flight_dict)

    # Create a dictionary with the cheapest_flights key
    result_dict = {"cheapest_flights": result}
...
def save_cheapest_flights_json(directory_path: str, output_dir: str, num_cheapest_flights: int = 3) -> str:
    ...
    try:
        ...
        json_data = merge_weekend_flight_data(
            directory_path, num_cheapest_flights)
        ...
        output_file = os.path.join(
            output_dir, f"cheapest_flights_{timestamp}.json")
...
def analyze_weekend_flight_trends(directory_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        csv_files = [f for f in os.listdir(
            directory_path) if f.endswith('.csv')]
        ...
        for file in csv_files:
            ...
            try:
                ...
                if len(date_parts) >= 4:
                    ...
                    dest, date = dest_date.split('_') if '_' in dest_date else (
                        dest_date.split('-')[0], dest_date)
                else:
